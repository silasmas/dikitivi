                        ...
                        <div class="row">
                            <div class="col-lg-10 col-sm-9 mx-auto">
                                <div class="ratio ratio-16x9">
                                    <iframe src="{{ $current_media->media_url }}?rel=0&showinfo=0" allow=""></iframe>
                                </div>
                            </div>
...
?>
    ...
    @if (!empty($current_media->media_description))
                                        <li class="text-muted fw-normal"><span>Description: </span> {{ $current_media->media_description }}</li>
    @endif
    ...
    @if (!empty($current_media->writer))
                                        <li class="text-muted fw-normal"><span>Film writer(s): </span> {{ $current_media->writer }}</li>
    @endif
    @if (!empty($current_media->director))
                                        <li class="text-muted fw-normal"><span>Film director(s): </span> {{ $current_media->director }}</li>
    @endif
