...
@section('app-content')
    ...
    @foreach ($medias as $media)
                                        <a href="{{ route('media.datas', ['id' => $media->id]) }}" class="list-group-item list-group-item-action position-relative">
                                            <img src="{{ !empty($media->cover_url) ? (!empty($_SERVER['HTTPS']) ? 'https' : 'http') . '://admin.dikitivi.com/public/storage' . $media->cover_url : asset('assets/img/blank-media-video.png') }}" alt="{{ $media->media_title }}" width="160" class="float-sm-start rounded-4 me-3">
