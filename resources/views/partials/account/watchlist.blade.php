                ...
                @foreach ($watchlist as $item)
                                        <a href="{{ route('media.datas', ['id' => $item->media->id]) }}" class="list-group-item list-group-item-action">
                                            <img src="{{ !empty($item->media->cover_url) ? $item->media->cover_url : asset('assets/img/blank-media-video.png') }}" alt="{{ $item->media->media_title }}" width="190" class="float-start rounded-4 me-3">
